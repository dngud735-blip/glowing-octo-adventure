...
async function processImage(input) {
    ...
    if (!apiKey) { 
        alert("먼저 설정(톱니바퀴)에서 API 키를 입력해주세요!"); 
        input.value = ''; 
        return; 
    }
    if (!file) return;

    showToast("사진 최적화 및 AI 분석 중... 🔍");
    document.getElementById('aiListContainer').classList.add('hidden');
    
    try {
        // 이미지 압축 (최대 폭 1024px)
        const compressedBase64 = await resizeImage(file, 1024);
        ...
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {
            ...
            body: JSON.stringify({
                contents: [{
                    parts: [
                        { text: "이 이미지에서 커팅 리스트(가로, 세로, 수량)를 찾아내서 JSON 배열로만 반환해. 형식: [{\"w\": 120, \"h\": 200, \"q\": 5}]. 다른 설명 없이 JSON만 출력해." },
                        { inline_data: { mime_type: "image/jpeg", data: compressedBase64.split(',')[1] } }
                    ]
                }]
            })
        });
        ...
        // JSON 부분만 정규식으로 추출 (설명이 섞여 나올 경우 대비)
        const jsonMatch = resultText.match(/\[\s*\{.*\}\s*\]/s);
        if (!jsonMatch) throw new Error("치수를 인식하지 못했습니다.");
        
        const items = JSON.parse(jsonMatch[0]);
        renderAiList(items, file);
        showToast("분석 완료! 리스트를 선택하세요.");
        
    } catch (err) {
        console.error("AI 에러:", err);
        showToast("인식 실패: 사진을 다시 찍거나 키를 확인하세요.");
    } finally {
        input.value = ''; 
    }
}
...
function calculate(isFromHistory = false) {
    ...
    if (wasteW === 99999 && wasteH === 99999) { showError("원단 폭이 부족합니다."); return; }
    ...
    document.getElementById('summaryList').innerHTML = `
        ...
        <div class="grid grid-cols-2 gap-2 mt-2">
            <div class="bg-violet-50/50 p-2 rounded-xl text-center">
                <span class="text-[11px] font-black text-violet-400 block mb-1 uppercase">Center</span>
                <span class="text-lg font-black text-violet-700">${fmtSpecial(FIX/2)}</span>
            </div>
            <div class="bg-emerald-50/50 p-2 rounded-xl text-center">
                <span class="text-[11px] font-black text-emerald-600 block mb-1 uppercase">3-Part</span>
            ...
            <div class="bg-blue-50/50 p-2 rounded-xl text-center">
                <span class="text-[11px] font-black text-blue-400 block mb-1 uppercase">4-Part</span>
            ...
            <div class="bg-amber-50/50 p-2 rounded-xl text-center">
                <span class="text-[11px] font-black text-amber-600 block mb-1 uppercase">Webbing</span>
    ...
    for (let n = 2; n <= 10; n++) {
        ...
        <div class="px-5 py-3 bg-slate-900 flex items-center justify-between">
            ...
            <span class="bg-blue-500 text-white px-3 py-1 rounded-lg font-black text-sm italic uppercase">${totalCutCount} Cuts</span>
        </div>
        <div class="p-5 bg-white">
            <div class="flex items-center justify-between mb-4">
                <span class="text-4xl font-black text-slate-900">${isGoal ? Math.ceil(Q/n)+'회' : n+'장'}</span>
            ...
            <div class="bg-blue-50 p-4 rounded-2xl text-center mb-6">
                <span class="text-[11px] font-black text-blue-400 block mb-1 uppercase tracking-widest">Total Length</span>
                <span class="text-5xl font-black text-blue-600">${fmt(totalLen)}<small class="text-2xl ml-1 text-slate-400">cm</small></span>
            </div>
            <div class="space-y-4">
                <div>
                    <span class="text-rose-500 font-black text-[11px] uppercase block mb-2 tracking-widest"><i class="fa-solid fa-cut mr-1"></i> Cut Points (1회 ${n}장)</span>
                ...
                <div>
                    <button onclick="toggleCardMarking(this)" class="w-full flex items-center justify-between py-1.5 text-slate-400">
                        <span class="font-black text-[11px] uppercase tracking-widest italic">Show Markers</span>
                    ...
                    <div class="hidden pt-2 space-y-3">
                        ...
                        ${pts.length > 0 ? `<div class="grid grid-cols-4 gap-2">${pts.map(p => `<span class="data-chip text-amber-700" ${getChipStyle(p)}>${p}</span>`).join('')}</div>` : ''}
                    </div>
                </div>
            </div>
        </div>`;
...
function saveToHistory(w, h, r, p1, p2, qg, dTotal, qLeft, pLabel) {
    ...
    if (historyData.length > 0 && historyData[0].w_in === w && historyData[0].h_in === h) return;
...
function renderHistory() {
    ...
    list.innerHTML = historyData.map(data => `
        <div class="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 history-card mb-3" onclick="fillFormFromHistory(${data.id})">
...
function toggleHistory() { 
    ...
    document.getElementById('historyIcon').style.transform = isHidden ? 'rotate(180deg)' : 'rotate(0deg)';
}
